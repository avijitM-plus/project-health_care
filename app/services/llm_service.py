...
from app.models.schemas import ImagingFindings, ConversationState
...
class LLMService:
    ...
    def generate_clinical_summary(self, state: ConversationState) -> dict:
        """Generate a structured clinical summary from the full session state."""
        import json as _json

        symptoms_text = ", ".join(
            f"{r.name.replace('_', ' ')}"
            + (f" ({r.severity})" if r.severity != "UNKNOWN" else "")
            + (f" — {r.duration}" if r.duration else "")
            for r in state.symptoms
        ) or "None reported"

        slots_text = _json.dumps(
            {k: v for k, v in state.clinical_slots.items() if v not in (None, "", "UNKNOWN")},
            indent=2
        )

        reports_text = "\n".join(
            f"  - {r.report_type} ({r.report_date}): {r.summary}"
            for r in state.reports
        ) or "  None uploaded"

        imaging_text = "\n".join(
            f"  - {s.filename}: {s.impression} [abnormalities: {', '.join(s.abnormalities) or 'none'}]"
            for s in state.imaging_studies
        ) or "  None uploaded"

        predictions_text = ", ".join(
            f"{p['name']} ({p.get('concern_level', '')})" for p in state.predictions
        ) or "None"

        user_prompt = (
            f"Generate a structured clinical summary for this session.\n\n"
            f"SYMPTOMS REPORTED:\n{symptoms_text}\n\n"
            f"CLINICAL SLOTS (established findings):\n{slots_text}\n\n"
            f"UPLOADED LAB REPORTS:\n{reports_text}\n\n"
            f"IMAGING STUDIES:\n{imaging_text}\n\n"
            f"PEAK URGENCY: {state.peak_urgency}\n"
            f"POSSIBLE CONDITIONS: {predictions_text}\n"
            f"CONVERSATION TURNS: {state.turn_count}\n"
            f"LONGITUDINAL TREND: {state.trend_summary or 'Not available'}\n"
        )
        ...
        return self._call_groq(
            ...
            fallback={
                ...
                "symptoms": [r.name for r in state.symptoms],
                "clinical_findings": state.clinical_slots,
                "uploaded_reports": [],
                "imaging_studies": [],
                "peak_urgency": state.peak_urgency,
                "possible_conditions": [p["name"] for p in state.predictions],
                "recommended_tests": [],
                "recommended_next_steps": "Please consult a licensed healthcare professional.",
                "conversation_turns": state.turn_count,
                "disclaimer": (
                    "This summary was generated by IASIS AI and is not a medical diagnosis. "
                    "It should be reviewed and validated by a licensed healthcare professional."
                ),
            },
        )
